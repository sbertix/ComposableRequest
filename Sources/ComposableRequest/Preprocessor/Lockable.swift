...
public extension Preprocessable where Self: Fetchable {
    ...
    /// - returns: A `Locker` unwrapping `self`.
    func locking(with unlocker: @escaping Locker<Self, AnyKey>.Unlocker) -> Locker<Self, AnyKey> {
        return locking(AnyKey.self, with: unlocker)
    }
}
...
public extension Preprocessable where Self: Fetchable, Preprocessed: HeaderComposable {
    ...
    /// - returns: A `Locker` unwrapping `self`.
    func locking<Secret>(_ secret: Secret.Type) -> Locker<Self, Secret> {
        return locking(Secret.self, with: { $0.append(header: HTTPCookie.requestHeaderFields(with: $1.cookies)) })
    }
    
    /// Return a `Locker` using the `header` as `Unlocker`.
    /// - returns: A `Locker` unwrapping `self`.
    func locking() -> Locker<Self, AnyKey> {
        return locking(AnyKey.self)
    }
}
