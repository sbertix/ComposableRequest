...
public extension Requestable {
    ...
    /// - parameters:
    ///     - preprocessor: An optional `Preprocessor`.
    ///     - process: A concrete type implementing `ResponseMappable`.
    ///     - pager: A valid `Pager`.
    /// - returns: A `Fetcher.Paginated` wrapping `self`.
    func prepare<Response: ResponseMappable>(preprocessor: Fetcher<Self, Response>.Preprocessor? = nil,
                                             process: Response.Type,
                                             pager: @escaping Fetcher<Self, Response>.Pager) -> Fetcher<Self, Response>.Paginated {
        return prepare(preprocessor: preprocessor,
                       processor: { $0.flatMap { data in Result { try Response(response: .init(ComposableRequest.Response.decode(data))) }}},
                       pager: pager)
    }
